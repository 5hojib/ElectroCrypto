...
static void aes256_set_decryption_key_portable(const uint8_t key[32], uint32_t expandedKey[60]) {
    uint32_t i, j, k, tmp;
...
static void aes256_encrypt_portable(const uint8_t in[16], uint8_t out[16], const uint32_t key[60]) {
...
static void aes256_decrypt_portable(const uint8_t in[16], uint8_t out[16], const uint32_t key[60]) {
    uint32_t s0, s1, s2, s3, t0, t1, t2, t3;
...
void aes256_set_encryption_key(const uint8_t key[32], uint32_t expandedKey[60]) {
    if (aesni == -1) aesni = has_aes_ni();
    if (aesni) {
        aes256_ni_set_encryption_key(key, expandedKey);
        return;
    }
    aes256_set_encryption_key_portable(key, expandedKey);
}
...
void aes256_set_decryption_key(const uint8_t key[32], uint32_t expandedKey[60]) {
    if (aesni == -1) aesni = has_aes_ni();
    if (aesni) {
        aes256_ni_set_decryption_key(key, expandedKey);
        return;
    }
    aes256_set_decryption_key_portable(key, expandedKey);
}

void aes256_encrypt(const uint8_t in[16], uint8_t out[16], const uint32_t key[60]) {
    if (aesni == -1) aesni = has_aes_ni();
    if (aesni) {
        aes256_ni_encrypt(in, out, key);
        return;
    }
    aes256_encrypt_portable(in, out, key);
}

void aes256_decrypt(const uint8_t in[16], uint8_t out[16], const uint32_t key[60]) {
    if (aesni == -1) aesni = has_aes_ni();
    if (aesni) {
        aes256_ni_decrypt(in, out, key);
        return;
    }
    aes256_decrypt_portable(in, out, key);
}
